...
import { createContext, useContext, createSignal, type JSX, type Component, Show } from 'solid-js';
import 'prism-themes/themes/prism-one-dark.css';
/** @ts-ignore */
import Prismjs from 'prismjs';
import 'prismjs/components/prism-typescript';
import { createMemo, mergeProps, splitProps, type ComponentProps, type ParentComponent } from 'solid-js';
...
export const SyntaxHighlighter: ParentComponent<Props> = (_props) => {
    const props = mergeProps({ language: 'javascript' }, _props);
    const [, rest] = splitProps(props, ['language', 'children', 'class', 'innerHTML']);
    ...
    const highlightedCode = createMemo<string | undefined>(() => {
        ...
        const grammar = Prismjs.languages[props.language] || Prismjs.languages.ts;
        if (!grammar) {
            return;
        }
    ...
    return (
        <pre class={`${languageClass()} ${props.class || ''}`}>
            <code class={`${languageClass()} ${props.class || ''}`} innerHTML={highlightedCode()} {...rest} />
        </pre>
    );
};
...
export const CodeBlock: Component<CodeBlockProps> = (props) => {
    return (
        <CodeBlockContext.Provider value={{ code: props.code }}>
            <div class={cn('relative overflow-hidden rounded-md border bg-background text-foreground', props.class)}>
                <div class="relative">
                    <SyntaxHighlighter language={props.language}>{props.code}</SyntaxHighlighter>
                    <Show when={props.children}>
                        <div class="absolute top-2 right-2 flex items-center gap-2">{props.children}</div>
                    </Show>
                </div>
            </div>
...
export const CodeBlockCopyButton: Component<CodeBlockCopyButtonProps> = (props) => {
    ...
    const copyToClipboard = async () => {
        ...
        try {
            ...
            setIsCopied(true);
            props.onCopy?.();
            setTimeout(() => setIsCopied(false), props.timeout ?? 2000);
        } catch (error) {
    ...
    return (
        <Button class={cn('shrink-0', props.class)} onClick={copyToClipboard} size="icon" variant="ghost" {...props}>
